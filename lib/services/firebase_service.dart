...
class FirebaseService {
  ...
  /// 플레이어 상태 동기화 - 실시간 점수, 콤보 등 업데이트
  Future<void> syncPlayerState(String roomId, String playerId, Map<String, dynamic> playerState) async {
    await _initialize();
    if (!_isInitialized || _firestore == null) {
      throw Exception('Firebase가 초기화되지 않았습니다.');
    }
    ...
    try {
      ...
      print('플레이어 상태 동기화 완료: $playerId - $playerState');
  ...
  /// 플레이어 상태 스트림 가져오기
  Stream<List<Map<String, dynamic>>> getPlayerStatesStream(String roomId) {
    if (!_isInitialized || _firestore == null) {
      return Stream.value([]);
    }
